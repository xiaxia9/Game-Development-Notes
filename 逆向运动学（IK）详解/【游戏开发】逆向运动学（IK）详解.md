...
如上图所示：$l_{1}$、$l_{2}$为两个杆，$\theta_{1}$、$\theta_{2}$ 表示旋转角度，可以看到图中两个杆可以有两种方案到达target点。由数学知识可知，其中一组解是：$\theta_{1}=cos^{-1}(\frac{l_{1}^{2}+x^{2}+y^{2}-l_{2}^{2}}{2l_{1}\sqrt{x^{2}+y^{2}}})$、$\theta_{2}=cos^{-1}(\frac{l_{1}^{2}+l_{2}^{2}-(x^{2}+y^{2})}{2l_{1}l_{2}})$
